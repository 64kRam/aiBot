...
import cadenaLogo from './assets/upcyberLogo.png';
...
const App = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const completions = await apiClient.createChatCompletion({
        model: "gpt-4",
        messages: [{ role: "user", content: `Make a LinkedIn post about: ${prompt} and add emojis and hashtags.` }],
        max_tokens: 2000,
        temperature: 0.7,
        top_p: 1,
        frequency_penalty: 0,
      ...
      setResponse(completions.data.choices[0].message.content);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 sm:p-0">
      <img src={cadenaLogo} alt="Upcyber Logo" className="h-12 mb-10 mt-40" />
      <form onSubmit={handleSubmit} className="w-full max-w-lg">
        <div className="flex items-center border-b-2 border-indigo-600 py-2">
          <input
            className="appearance-none bg-transparent border-none w-full text-white mr-3 py-1 px-2 leading-tight focus:outline-none"
            type="text"
            placeholder="Enter a topic you want to Post to LinkedIn about."
            value={prompt}
